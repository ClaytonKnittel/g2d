...
int font_init(font_t *f, const char *font_path, uint64_t font_height) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, f->tex_width, f->tex_height, 0, GL_RED,
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      FONT_MAX_STR_LEN * VERTS_PER_CHAR * DATA_PER_VERT * sizeof(float), NULL,
      GL_DYNAMIC_DRAW);
...
static void _font_render(font_t *f, const char *text, float x_pos, float y_pos,
  ...
  float color_vec[4] = { color_r(f->font_color), color_g(f->font_color),
                         color_b(f->font_color), color_a(f->font_color) };
