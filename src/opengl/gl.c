...
static void _window_resize_cb(GLFWwindow* w, int width, int height) {
  gl_context_t* c = _get_window_context(w);
  glfwGetFramebufferSize(w, &width, &height);

  width_height wh = { .w = width, .h = height };
  c->wh = wh;
}
...
void gl_set_bg_color(color_t color) {
  glClearColor(color_r(color), color_g(color), color_b(color), color_a(color));
}

void gl_register_key_callback(gl_context_t* c,
                              void (*callback)(gl_context_t*, int key,
                                               int action, int scancode,
                                               int mods)) {
