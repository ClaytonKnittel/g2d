...
static double waveform(double t, double f, double mix) {
  double res1 = 0;
  ...
  for (uint64_t i = 1; i < 60; i++) {
    uint64_t newi = 2 * i - 1;
    res2 += (i % 2 == 1 ? -1. : 1) / (newi * newi) *
            sin(M_PI_2 + 2 * M_PI * t * f * newi);
...
void check_err(ALCdevice* device) {
  ALCenum error = alcGetError(device);
  switch (error) {
    case ALC_INVALID_VALUE:
      printf(
          "ALC_INVALID_VALUE: an invalid value was passed to an OpenAL "
          "function\n");
      break;
    ...
    case ALC_INVALID_ENUM:
      printf(
          "ALC_INVALID_ENUM: an unknown enum value was passed to an OpenAL "
          "function\n");
      break;
...
int al_test() {
  ...
  alBufferData(buffer, AL_FORMAT_STEREO16, buf,
               2 * DURATION * FREQUENCY * sizeof(int16_t), FREQUENCY);
