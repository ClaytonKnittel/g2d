...
class Renderer {
 private:
  ...
  void getShaders() {
    ...
    dispatch_data_t data = dispatch_data_create(
        g_metal_shader_lib_source, g_metal_shader_lib_source_size, nullptr,
        ^void(void){
        });
    ...
    MTL::Function *vertex_fn = m_library->newFunction(
        NS::String::string("rayVertex", NS::UTF8StringEncoding));
    MTL::Function *fragment_fn = m_library->newFunction(
        NS::String::string("rayFragment", NS::UTF8StringEncoding));
  ...
  void buildTextures() {
    ...
    m_texture->replaceRegion(MTL::Region(0, 0, tw, th), 0, texture_data,
                             tw * 4);
  ...
  void buildBuffers() {
    ...
    m_v_index_buf =
        m_device->newBuffer(sizeof(v_index), MTL::ResourceStorageModeManaged);
    ...
    const size_t instance_buf_len =
        NUM_INSTANCES * sizeof(shader_types::InstanceData);
    ...
    m_library->newFunction(NS::String::string("rayVertex",
                            NS::UTF8StringEncoding));
    MTL::ArgumentEncoder* arg_encoder = vertex_fn->newArgumentEncoder(0);
    m_arg_buf = m_device->newBuffer(arg_encoder->encodedLength(),
    MTL::ResourceStorageModeManaged); if (m_arg_buf == nullptr) { printf("Failed
    to initialize arg buf\n"); exit(-1);
    }
 ...
 public:
  ...
  void Render(MTK::View *view) {
    ...
    MTL::Buffer *instance_buf_ptr =
        m_instance_buf[frame % MAX_FRAMES_IN_FLIGHT];
    ...
    instance_buf_ptr->didModifyRange(
        NS::Range::Make(0, instance_buf_ptr->length()));
    ...
    encoder->drawIndexedPrimitives(
        ...
        MTL::IndexType::IndexTypeUInt16, m_v_index_buf, NS::UInteger(0),
...
class AppDelegate : public NS::ApplicationDelegate {
 ...
 public:
  AppDelegate()
      : m_device(nullptr), m_view(nullptr), m_view_delegate(nullptr) {}
  ~AppDelegate() {
  ...
  virtual void applicationWillFinishLaunching(
    ...
    NS::Application *app =
        static_cast<NS::Application *>(notification->object());
  ...
  virtual void applicationDidFinishLaunching(
    ...
    m_window->setTitle(
        NS::String::string("test program!", NS::UTF8StringEncoding));
