...
- (void)setup
{
  ...
  dispatch_data_t dat = dispatch_data_create(g_metal_shader_lib_source, g_metal_shader_lib_source_size, NULL, NULL);
  ...
  // Create vertices.
  struct Vertex verts[] = {
      {{-0.5, -0.5, 0}, {255, 0, 0, 255}}, {{0, 0.5, 0}, {0, 255, 0, 255}}, {{0.5, -0.5, 0}, {0, 0, 255, 255}}};
  char buf[sizeof(verts)];
  memcpy(buf, verts, sizeof(verts));
  _vertexBuffer = [self.device newBufferWithBytes:buf length:sizeof(verts) options:MTLResourceStorageModeShared];

  // Create uniform buffers.
  for (int i = 0; i < uniformBufferCount; i++) {
    _uniformBuffers[i] = [self.device newBufferWithLength:64 options:MTLResourceCPUCacheModeWriteCombined];
  }
  frame = 0;
